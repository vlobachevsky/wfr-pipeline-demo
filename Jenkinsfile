...
private void updateDB() {
    ...
    bat 'java -showversion -Xms512m -Xmx1024m -Xss1m -classpath ".\\web\\WEB-INF\\classes;.\\web\\WEB-INF\\lib\\*; " RunSQL delay=0 output.result=0 output.sql=0 error.handling=EXIT output.verbose=1 uri=jdbc:sqlserver://%DB_SERVER_NAME%:1433;DatabaseNameP=Zeyt;encrypt=false user=sa password=Admin1234 input.file=sql\\DBUpdateCurrentSprint.txt  jdbc.driver=com.microsoft.sqlserver.jdbc.SQLServerDriver'
    bat 'java -showversion -Xms512m -Xmx1024m -Xss1m -classpath ".\\web\\WEB-INF\\classes;.\\web\\WEB-INF\\lib\\*; " RunSQL delay=0 output.result=0 output.sql=0 error.handling=EXIT output.verbose=1 uri=jdbc:sqlserver://%DB_SERVER_NAME%:1433;DatabaseNameP=Zeyt;encrypt=false user=sa password=Admin1234 input.file=sql\\DBUpdate.txt  jdbc.driver=com.microsoft.sqlserver.jdbc.SQLServerDriver'
}
